...
import { ChartPieSlice, CheckCircle, WarningCircle, ListBullets, Printer } from "@phosphor-icons/react";
...
export function TSModReportes() {
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/moderator/reports/technical-skills`, {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard title="Total Tecnologías" value={data.total} icon={<ListBullets size={32} />} color="text-indigo-500" bgColor="bg-indigo-500/10" />
        <StatCard title="Habilitadas" value={data.active} icon={<CheckCircle size={32} />} color="text-emerald-500" bgColor="bg-emerald-500/10" />
        <StatCard title="Deshabilitadas" value={data.inactive} icon={<WarningCircle size={32} />} color="text-amber-500" bgColor="bg-amber-500/10" />
      </div>

      <div className="bg-[#13152e] border border-[#232555] rounded-3xl p-8">
        <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2"><ChartPieSlice size={20} className="text-[#6c72ff]" /> Distribución por Categoría</h3>
          ...
          {data.categories.map((cat) => (
            <div key={cat.category} className="flex items-center gap-4">
              ...
              <div className="grow h-2 bg-slate-900 rounded-full overflow-hidden">
                <div className="h-full bg-[#6c72ff] rounded-full" style={{ width: `${data.total > 0 ? (cat.total / data.total) * 100 : 0}%` }} />
              </div>
...
function StatCard({ title, value, icon, color, bgColor }: { title: string, value: number, icon: JSX.Element, color: string, bgColor: string }) {
  return (
