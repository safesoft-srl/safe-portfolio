    ...
    >
      <InputGroup className="w-full">
        <ComboboxPrimitive.Input
          render={
            <InputGroupInput disabled={disabled} placeholder={selectedLabel || placeholder} />
          }
        />

        <InputGroupAddon align="inline-end">
          <InputGroupButton size="icon-xs" variant="ghost" onClick={() => setOpen((v) => !v)}>
            <CaretDownIcon className="size-4" />
          </InputGroupButton>

          {value && (
            <InputGroupButton size="icon-xs" variant="ghost" onClick={() => onChange("")}>
              <XIcon />
            </InputGroupButton>
          )}
        </InputGroupAddon>
      ...
      <ComboboxPrimitive.Portal>
        <ComboboxPrimitive.Positioner className="z-50">
          <ComboboxPrimitive.Popup className="mt-2 max-h-72 w-[--anchor-width] overflow-auto rounded-md border dark:bg-[#1f2552] p-1 shadow-md">
            {options.length === 0 && (
