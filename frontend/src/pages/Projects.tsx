    ...
    return (
        <>
            <AlertDialog open={open} onOpenChange={setOpen}>
                        ...
                        {projects.length === 0 ? (
                            ...
                            <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {projects.map((project, idx) => {
                                    ...
                                    const skillIds: number[] = Array.isArray(project.skill_ids) ? project.skill_ids : [];
                                    return (
                                                            ...
                                                            <div className="flex gap-2 ml-3 shrink-0">
                                                                <button
                                                                    ...
                                                                    onClick={() => {
                                                                        ...
                                                                        let skillIds: number[] = [];
                                                                        if (Array.isArray(project.skill_ids) && project.skill_ids.length > 0) {
                                                                            skillIds = project.skill_ids;
                                                                        } else if (Array.isArray(project.skill_projects) && project.skill_projects.length > 0) {
                                                                            skillIds = project.skill_projects
                                                                                .map(sp => {
                                                                                    const skill = skills.find(s => s.skill_name === sp.skill_name);
                                                ...
                                                <div className="flex flex-wrap gap-2 mb-4">
                                                    {Array.isArray(project.skill_projects) && project.skill_projects.length > 0 ? (
                                                        project.skill_projects.map((sp, i) => (
                                                            <span key={i} className="bg-[#6c72ff] text-white text-xs px-3 py-1 rounded-full">
                                                                {sp.skill_name}
                                                            </span>
                                                        ))
                                                    ) : skillIds.length > 0 ? (
                                                ...
                                                <div className="flex gap-3 mb-2">
                                                    {project.url_github && (
                                                        <a href={project.url_github} target="_blank" rel="noopener noreferrer">
                                                            <Button variant="outline" className="flex items-center gap-2"><span className="i-mdi-github" />GitHub</Button>
                                                        </a>
                                                    )}
                                                    {project.url_demo && (
                                                        <a href={project.url_demo} target="_blank" rel="noopener noreferrer">
                                                            <Button variant="outline" className="flex items-center gap-2"><span className="i-mdi-link-variant" />Ver Demo</Button>
                                                        </a>
                                                    )}
                                                </div>
                                            </div>
                ...
                <AlertDialogContent
                    className="max-w-5xl w-full rounded-2xl border-sidebar-border bg-sidebar px-10 py-8 text-sidebar-foreground max-h-[85vh] overflow-y-auto sm:max-h-none sm:overflow-visible"
                >
                    ...
                    <form className="text-left" onSubmit={async e => {
                        ...
                        try {
                            ...
                            showErrorToast(editIndex !== null ? "Error al actualizar proyecto" : "Error al registrar proyecto");
                        } finally {
                            setIsSaving(false);
                        }
                    }}>
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                    ...
                                                    <div className="flex flex-col gap-4">
                                                        <div className="space-y-1.5">
                                                            <Label htmlFor="projectName" className="text-xs font-semibold text-slate-900 dark:text-slate-300">Nombre del Proyecto *</Label>
                                                            <Input
                                                                id="projectName"
                                                                name="projectName"
                                                                value={form.name}
                                                                ...
                                                                onBlur={e => setErrors(prev => ({ ...prev, name: e.target.value.trim() ? "" : "El nombre del proyecto es obligatorio." }))}
                                                            ...
                                                            />
                                                            {errors.name && (
                                                                <p className="text-xs mt-1" style={{ color: "var(--destructive)" }}>{errors.name}</p>
                                                            )}
                                                        </div>
                                                        <div className="space-y-1.5">
                                                            <Label htmlFor="projectDescription" className="text-xs font-semibold text-slate-900 dark:text-slate-300">Descripción *</Label>
                                                            <textarea
                                                                ...
                                                                onBlur={e => setErrors(prev => ({ ...prev, description: e.target.value.trim() ? "" : "La descripción es obligatoria." }))}
                                                                ...
                                                                style={{ scrollbarColor: '#23234a #181c2f', scrollbarWidth: 'thin', minHeight: '96px', maxHeight: '96px' }}
                                                            />
                                                            {errors.description && (
                                                                <p className="text-xs mt-1" style={{ color: "var(--destructive)" }}>{errors.description}</p>
                                                            )}
                                                            <style>{`
                                                        ...
                                                        <div className="-mt-2">
                                                            ...
                                                            />
                                                            {errors.skill_ids && (
                                                                <p className="text-xs mt-1" style={{ color: "var(--destructive)" }}>{errors.skill_ids}</p>
                                                            )}
                                                        </div>
                                                    </div>
                                                    {/* Columna Derecha */}
                                                    <div className="flex flex-col gap-4">
                                                        <div className="space-y-1.5">
                                                            <Label htmlFor="urlGithub" className="text-xs font-semibold text-slate-900 dark:text-slate-300">URL GitHub</Label>
                                                            <Input
                                                                id="urlGithub"
                                                                name="urlGithub"
                                                                ...
                                                                onBlur={e => setErrors(prev => ({ ...prev, url_github: e.target.value.trim() ? "" : "La URL de GitHub es obligatoria." }))}
                                                            ...
                                                            />
                                                            {errors.url_github && (
                                                                <p className="text-xs mt-1" style={{ color: "var(--destructive)" }}>{errors.url_github}</p>
                                                            )}
                                                        </div>
                                                        <div className="space-y-1.5">
                                                            <Label htmlFor="urlDemo" className="text-xs font-semibold text-slate-900 dark:text-slate-300">URL Demo</Label>
                                                            <Input
                                                                id="urlDemo"
                                                                name="urlDemo"
                                                                value={form.url_demo}
                        ...
                        <AlertDialogFooter className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                            ...
                            >
                                {isSaving ? (
                                    ...
                                    editIndex !== null ? "Guardar Cambios" : "Agregar Proyecto"
                                )}
                            </Button>
